...
for auth_url in $(curl "$order_url" | jq -r .authorizations[]) ; do
	curl "$auth_url" > .authz
	cat .authz | jq '.challenges[] | select(.type | test("dns"))' > .challenge
	identifier=$(cat .authz | jq -r .identifier.value)
	token=$(cat .challenge | jq -r .token)
	...
	challenge_url=$(cat .challenge | jq -r .url)
